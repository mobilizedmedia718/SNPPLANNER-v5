...
const CheckInUI = {
    ...
    async processBarcode(rawValue) {
        try {
            ...
            let result;
            try { result = await Eventbrite.registerScannedPatron(this.eventId, value); }
            catch (firstError) {
            ...
            if (!checkIns.some(x => x.attendeeId === result.attendee.id)) {
                checkIns.push({ attendeeId: result.attendee.id, customerId: result.customer.id, barcode: result.attendee.barcode, checkedInAt: new Date().toISOString(), source: "SNP Planner Scanner" });
        ...
        } catch (error) { alert(error?.message || "Ticket could not be checked in."); }
    },
    ...
    async startScanner() {
        ...
        // Prefer the browser's native detector when available; otherwise use html5-qrcode.
        ...
        if (!loaded) return alert("The camera scanner could not load. You can still enter the ticket barcode manually.");
        try {
            ...
            await this.html5Scanner.start(
                ...
                async decodedText => { if (!this.html5Scanner) return; await this.processBarcode(decodedText); },
                () => {}
            );
        } catch (error) {
            await this.stopScanner();
            alert("Unable to open the camera. Allow camera permission and try again, or enter the ticket barcode manually.");
        }
    },
