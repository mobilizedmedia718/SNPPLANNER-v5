...
(function(){
  ...
  const WINE_NAMES = [
    "Barefoot Cabernet Sauvignon","Barefoot Moscato","Josh Cellars Cabernet Sauvignon","Josh Cellars Chardonnay",
    "Sutter Home White Zinfandel","Sutter Home Moscato","Kendall-Jackson Vintner's Reserve Chardonnay","La Marca Prosecco",
    "Stella Rosa Black","Stella Rosa Rosso","Woodbridge Cabernet Sauvignon","Woodbridge Chardonnay"
  ];
  ...
  const EventMenu = {
    ...
    library(){
      const byKey=new Map();
      [...this.systemPresets(),...this.templates()].forEach(item=>byKey.set(`${String(item.category||"").trim().toLowerCase()}|${String(item.name||"").trim().toLowerCase()}`,item));
      return [...byKey.values()];
    },
    ensureEvent(event){
      if(!event) return null;
      if(!Array.isArray(event.menuItems)){ event.menuItems=[]; Events.update(event.id,{menuItems:event.menuItems}); }
      return event;
    },
    categories(){ return [...new Set(["Bottles","Glasses","Entree","Food","Snack","Dessert","Non-Alcoholic","Beverage","Special",...this.library().map(x=>String(x.category||"").trim()).filter(Boolean)])].sort(); },
    publicUrl(eventId){ return `https://mobilizedmedia718.github.io/SNPPLANNER-v5/preorder.html?event_id=${encodeURIComponent(eventId)}`; },
    qrPosterUrl(eventId){ return `https://mobilizedmedia718.github.io/SNPPLANNER-v5/menu-qr.html?event_id=${encodeURIComponent(eventId)}`; },
    activeItems(event){ return (event.menuItems||[]).filter(x=>x.active!==false&&String(x.name||"").trim()&&Number(x.price||0)>0); },
    finalize(eventId){
      const event=this.ensureEvent(Events.get(eventId)); if(!event) return;
      const items=this.activeItems(event);
      if(!items.length) return alert("Add at least one active menu item with a price before finalizing the menu.");
      const snapshot=JSON.parse(JSON.stringify(items));
      const stamp=new Date().toISOString();
      Events.update(eventId,{menuFinalized:true,menuFinalizedAt:stamp,finalizedMenuItems:snapshot,menuPublicUrl:this.publicUrl(eventId)});
      alert("Menu finalized. The public ordering QR is ready to print.");
      UI.renderEvents();
    },
    reopen(eventId){
      const event=Events.get(eventId); if(!event) return;
      if(!confirm("Reopen this menu for editing? The existing printed QR will still work, but the public menu will not update until you finalize again.")) return;
      Events.update(eventId,{menuFinalized:false}); UI.renderEvents();
    },
    ...
    addFromTemplate(eventId,templateId){
      ...
      const event=this.ensureEvent(Events.get(eventId)); if(!event) return;
      if(event.menuFinalized) return alert("Reopen the finalized menu before changing it.");
      event.menuItems.push({...template,id:Utils.id(),quantity:0,active:true,preset:false}); Events.update(eventId,{menuItems:event.menuItems}); UI.renderEvents();
    },
    updateItem(eventId,itemId,key,value){
      ...
      item[key]=["price","quantity"].includes(key)?Number(value||0):["active","includedWithVip"].includes(key)?!!value:value; Events.update(eventId,{menuItems:event.menuItems});
    },
    removeItem(eventId,itemId){
      const event=this.ensureEvent(Events.get(eventId)); if(!event) return;
      if(event.menuFinalized) return alert("Reopen the finalized menu before changing it.");
      event.menuItems=event.menuItems.filter(x=>x.id!==itemId); Events.update(eventId,{menuItems:event.menuItems}); UI.renderEvents();
    },
    saveTemplate(eventId,itemId){
      ...
      const list=this.templates(); const existing=list.find(x=>String(x.name||"").trim().toLowerCase()===String(item.name||"").trim().toLowerCase()&&String(x.category||"").trim().toLowerCase()===String(item.category||"").trim().toLowerCase());
      ...
      if(existing) Object.assign(existing,payload); else list.push(payload); this.saveTemplates(list); alert("Menu item saved to the reusable menu library."); UI.renderEvents();
    },
    rows(event){
      event=this.ensureEvent(event); if(!event.menuItems.length) return `<p>No menu items assigned to this event yet. Choose only the wines and foods you want for this event from the library above.</p>`;
      const cats=this.categories(), disabled=event.menuFinalized?"disabled":"";
    ...
    libraryOptions(category){ return this.library().filter(x=>!category||x.category===category).map(t=>`<option value="${UI.esc(t.id)}">${UI.esc(t.name)} — ${Utils.money(Number(t.price||0))}</option>`).join(""); },
    card(event){
      event=this.ensureEvent(event); const link=this.publicUrl(event.id), poster=this.qrPosterUrl(event.id);
      return `<div class="card"><h3>Event Food & Beverage Menu</h3>
        ${event.menuFinalized?`<div style="padding:12px;border:1px solid #9ccfae;background:#edf9f1;border-radius:10px"><strong>FINALIZED</strong>${event.menuFinalizedAt?` — ${new Date(event.menuFinalizedAt).toLocaleString()}`:""}<br>This frozen menu is what customers see from the printed QR.</div>`:`<p>Build only the menu you want to offer at this event. The library remembers reusable items, while event prices, quantities, and descriptions remain editable.</p>`}
        ${event.menuFinalized?"":`<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:10px;align-items:end;">
    ...
    open(eventId){ const event=this.ensureEvent(Events.get(eventId)); if(!event) return; const ws=document.getElementById("workspace"); if(ws) ws.innerHTML=`<h2>Menu — ${UI.esc(event.name||"Event")}</h2>${this.card(event)}${window.LiveEvent?.activeId?`<button onclick="LiveEvent.enter('${UI.esc(eventId)}')">Back to Live Event</button>`:""}`; }
  };
  window.EventMenu=EventMenu;
  const priorEvents=UI.renderEvents;
  UI.renderEvents=function(...args){ const result=priorEvents.apply(this,args); const workspace=document.getElementById("workspace"); if(!workspace) return result; Events.all().forEach(event=>{ const cards=[...workspace.querySelectorAll(".card")]; const card=cards.find(c=>c.querySelector(`input[onchange*="Events.update('${event.id}'"]`)); if(card&&!document.getElementById(`event-menu-${event.id}`)){ const quick=document.createElement("button"); quick.type="button"; quick.textContent="Menu"; quick.onclick=()=>EventMenu.open(event.id); card.appendChild(quick); const wrap=document.createElement("div"); wrap.id=`event-menu-${event.id}`; wrap.innerHTML=EventMenu.card(event); card.insertAdjacentElement("afterend",wrap); } }); return result; };
})();