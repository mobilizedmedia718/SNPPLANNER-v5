const Venues = {
    ...
    fullAddress(venue) {
        if (!venue) return "";
        return [venue.address, venue.address2, venue.city, venue.state, venue.zip, venue.country]
            .filter(Boolean)
            .join(", ");
    },

    totalRentalCost() {
        return this.list.reduce((sum, v) => sum + Number(v.rentalCost || 0), 0);
    },

    addOffering(venueId) {
        const venue = this.get(venueId);
        if (!venue) return;
        if (!Array.isArray(venue.offerings)) venue.offerings = [];
    ...
    updateOffering(venueId, offeringId, updates) {
        ...
        const offering = venue.offerings.find(item => item.id === offeringId);
        if (!offering) return;
    ...
    removeOffering(venueId, offeringId) {
        const venue = this.get(venueId);
        if (!venue || !Array.isArray(venue.offerings)) return;
        venue.offerings = venue.offerings.filter(item => item.id !== offeringId);
        this.save();
    },
    ...
    offeringNames() {
        return [
            ...new Set(
                this.list
                    .flatMap(venue => Array.isArray(venue.offerings) ? venue.offerings : [])
