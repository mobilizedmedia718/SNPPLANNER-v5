...
(function () {
    ...
    const UNRESOLVED_STATUSES = new Set(["Pending", "Unpaid", "Deferred", "On Hold"]);
    const EXCLUDED_STATUSES = new Set(["Cancelled", "Refunded", "Transferred"]);

    function esc(value) { return UI.esc(value); }
    function money(value) { return Utils.money(Number(value || 0)); }

    function eventRows(eventId) {
        return Finance.byEvent(eventId).filter(t => !EXCLUDED_STATUSES.has(String(t.status || "")));
    }

    function totals(eventId) {
        const rows = eventRows(eventId);
        const collectedIncome = rows.filter(t => t.type === "Income" && PAID_STATUSES.has(String(t.status || "Completed")))
            .reduce((s,t) => s + Number(t.amount || 0), 0);
        const pendingIncome = rows.filter(t => t.type === "Income" && UNRESOLVED_STATUSES.has(String(t.status || "")))
            .reduce((s,t) => s + Number(t.amount || 0), 0);
        const expenses = rows.filter(t => t.type === "Expense" && !UNRESOLVED_STATUSES.has(String(t.status || "")))
            .reduce((s,t) => s + Number(t.amount || 0), 0);
        const pendingExpenses = rows.filter(t => t.type === "Expense" && UNRESOLVED_STATUSES.has(String(t.status || "")))
            .reduce((s,t) => s + Number(t.amount || 0), 0);
        const taxes = rows.filter(t => PAID_STATUSES.has(String(t.status || "Completed")))
            .reduce((s,t) => s + Number(t.taxAmount || 0), 0);
        const refunds = Finance.byEvent(eventId).filter(t => String(t.status || "") === "Refunded")
            .reduce((s,t) => s + Number(t.amount || 0), 0);
        return {
            collectedIncome,
            pendingIncome,
            ...
            projectedProfit: (collectedIncome + pendingIncome) - (expenses + pendingExpenses)
        };
    }
    ...
    function guestStats(event) {
        ...
        const confirmed = guests.filter(g => String(g.status || "Confirmed") !== "Cancelled").length;
        const checkedIn = guests.filter(g => g.checkedIn || String(g.status || "").toLowerCase() === "checked in").length;
        const tickets = guests.reduce((s,g) => s + Number(g.ticketQuantity || 0), 0);
    ...
    function unresolvedHtml(eventId) {
        const unresolved = Finance.byEvent(eventId).filter(t => UNRESOLVED_STATUSES.has(String(t.status || "")));
        if (!unresolved.length) return `<p style="color:#166534;"><strong>No unresolved event payments.</strong></p>`;
        return `
    ...
    UI.resolveEventPayment = function(eventId, transactionId, status) {
        ...
        if (status === "Refunded") updates.notes = `${tr.notes || ""}${tr.notes ? "\n" : ""}Resolved as refund during event closeout.`;
        Finance.update(transactionId, updates);
        UI.openEventCloseout(eventId);
    };

    UI.transferEventPayment = function(eventId, transactionId) {
        const targets = Events.all().filter(e => e.id !== eventId && e.status !== "Completed" && e.status !== "Cancelled");
        if (!targets.length) return alert("There is no other open event to transfer this payment to.");
        const names = targets.map((e,i) => `${i+1}. ${e.name || "Untitled Event"}${e.date ? ` — ${e.date}` : ""}`).join("\n");
        const choice = prompt(`Transfer to which event? Enter the number:\n\n${names}`);
    ...
    UI.confirmEventCloseout = function (eventId) {
        ...
        if (!event) return;
        const checks = Array.from(document.querySelectorAll("#eventCloseoutModal .closeout-check"));
        if (!checks.length || !checks.every(c => c.checked)) return alert("Check every item before completing this event.");

        const t = totals(eventId);
        const unresolved = Finance.byEvent(eventId).filter(tr => UNRESOLVED_STATUSES.has(String(tr.status || "")));
