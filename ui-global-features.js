...
(function () {
  ...
  function isInlineEditorButton(button) {
    ...
    return (
      /^(?:\+\s*)?Add Product \/ Service$/i.test(text) ||
  ...
  function captureEditorFields(root) {
    const snapshot = {};
    root
  ...
  function restoreEditorFields(snapshot) {
  ...
  function showUnsavedModal() {
    ...
    overlay.style.cssText =
      "position:fixed;inset:0;background:rgba(0,0,0,.52);display:flex;align-items:center;justify-content:center;z-index:999999;padding:20px;";
    overlay.innerHTML = `
  ...
  document.addEventListener(
    ...
    function (event) {
      ...
      if (
        !button.closest("#unsavedChangesModal") &&
        (isSaveButton(button) || isCancelButton(button))
      ) {
        markClean();
        return;
      }
  ...
  function wrapRender(name) {
    ...
    if (typeof original !== "function" || original.__snpSharedOptionsWrapped)
      return;

    const wrapped = function (...args) {
      // Dynamic nested rows rerender the same editor. Preserve every
      // unsaved top-level value instead of only the focused field.
      const preserveDraft = /Edit$/.test(name) && workspaceDirty;
      const draft = preserveDraft ? captureEditorFields(workspace()) : null;
      if (!preserveDraft) markClean();
      const result = original.apply(UI, args);
      setTimeout(() => {
        if (draft) restoreEditorFields(draft);
        installSharedSuggestions();
      }, 0);
      return result;
    };
