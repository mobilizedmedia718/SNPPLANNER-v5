...
(function () {
    ...
    document.getElementById("workspace").innerHTML = `
            ...
            <div class="card">
                ...
                <div id="eventbriteStatus" class="status" style="margin-top:14px;">
                    ${link?.lastSync ? `Last synchronized: ${UI.esc(Utils.formatDateTime(link.lastSync))}` : "Not synchronized yet."}
                </div>
            </div>
  ...
  UI.saveEventbriteConnection = function () {
    const eventId =
      document.getElementById("eventbritePlannerEvent")?.value || "";
    if (!eventId) return;
    Eventbrite.setConnectorUrl(
      document.getElementById("eventbriteConnectorUrl")?.value || "",
    );
    Eventbrite.setEventbriteEventId(
      eventId,
  ...
  UI.importEventbriteTicketTypes = async function () {
    ...
    try {
      ...
      if (nextStatus)
        nextStatus.textContent = "Ticket types imported successfully.";
    } catch (error) {
