...
(function () {
  ...
  const EmployeeDirectory = {
    portalUrl: "https://mobilizedmedia718.github.io/SNPPLANNER-v5/staff.html",
    employees: [],
    async request(options = {}) {
      const s = await SNPDatabase.getSession();
      if (!s?.access_token) throw new Error("Sign in again first.");
      const r = await fetch(
        `${SNP_SUPABASE_URL}/functions/v1/employee-directory-api`,
        {
          ...options,
          headers: {
            ...SNPDatabase.headers(s.access_token),
            ...(options.headers || {}),
          },
        },
      );
      ...
      if (!r.ok || d.ok === false)
        throw new Error(d.error || "Employee directory request failed.");
      return d;
    },
    ...
    async saveManual() {
      try {
        ...
        alert(
          "Employee saved. You can now send their Staff Portal invitation by email.",
        );
      } catch (e) {
        alert(e.message);
      }
    ...
    async sendInvite(employeeId) {
      try {
        ...
        if (!emp.email)
          return alert(
            "Add an email address before sending a Staff Portal invite.",
          );
        const d = await this.request({
          method: "POST",
          body: JSON.stringify({ action: "send_invite", employeeId }),
        });
        await this.refresh(true);
        alert(d.message || "Staff invitation sent.");
      } catch (e) {
        alert("Unable to send staff invitation: " + e.message);
      }
    },
    async createInvite() {
      try {
        const email =
            document.getElementById("inviteEmployeeEmail")?.value.trim() || "",
          defaultRole =
            document.getElementById("inviteEmployeeRole")?.value.trim() ||
            "Event Staff";
        const d = await this.request({
          method: "POST",
          body: JSON.stringify({ action: "create_invite", email, defaultRole }),
        });
        const out = document.getElementById("employeeInviteResult");
        if (out)
          out.innerHTML = `<p><strong>Backup signup link</strong></p><input readonly value="${UI.esc(d.inviteUrl)}" onclick="this.select()"><p><small>Expires ${UI.esc(Utils.formatDateTime(d.expiresAt))}. Use this only if you want to send the link manually.</small></p>`;
      } catch (e) {
        alert(e.message);
      }
    },
    async copyInvite(employeeId) {
      try {
        ...
        if (!emp?.email) return alert("Employee email is required.");
        const d = await this.request({
          method: "POST",
          body: JSON.stringify({
            action: "create_invite",
            email: emp.email,
            defaultRole: emp.default_role || "Event Staff",
          }),
        });
        await navigator.clipboard.writeText(d.inviteUrl);
        alert("Backup staff signup link copied.");
      } catch (e) {
        alert(e.message);
      }
    },
    async addToEvent(eventId, employeeId) {
      try {
        ...
        if (!event) return;
        event.staffAssignments = Array.isArray(event.staffAssignments)
          ? event.staffAssignments
          : [];
        const existing = event.staffAssignments.find(
          (x) =>
            String(x.employeeProfileId || "") === String(emp.id) ||
            String(x.staffEmail || "").toLowerCase() ===
              String(emp.email || "").toLowerCase(),
        );
        if (existing)
          return alert("This employee is already assigned to this event.");
        const row = {
          id: Utils.id(),
    ...
    async loadForEvent(eventId) {
      ...
      UI.renderEvents();
      setTimeout(() => {
        const el = document.getElementById(`employee-dir-${eventId}`);
        if (el) el.focus();
      }, 50);
    },
    options() {
    ...
    status(x) {
      const s = String(x.invite_status || "not_sent");
      if (s === "connected" || x.invite_accepted_at) return "Portal Connected";
      if (s === "sent") return "Invite Sent";
      return x.staff_user_id ? "Portal Account Found" : "Invite Not Sent";
    },
  };
  ...
  UI.renderSidebar = function () {
    ...
    if (s && !document.getElementById("employeesMainMenuBtn")) {
      const b = document.createElement("button");
      b.id = "employeesMainMenuBtn";
      b.textContent = "Employees / Event Staff";
      b.onclick = async () => {
        UI.renderEmployees();
        await EmployeeDirectory.refresh(true);
      };
      const eventsBtn = [...s.querySelectorAll("button")].find(
        (x) => x.textContent.trim() === "Events",
      );
      if (eventsBtn) eventsBtn.insertAdjacentElement("afterend", b);
