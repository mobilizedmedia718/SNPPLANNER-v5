...
(function () {
  ...
  const LiveEvent = {
    ...
    get activeEvent() {
      return this.activeId ? Events.get(this.activeId) : null;
    },
    ...
    endEvent(eventId) {
      ...
      if (!event.eventStartedAt)
        return alert("Mark the event as started first.");
      const stamp = nowIso();
      Events.update(eventId, {
        eventEndedAt: event.eventEndedAt || stamp,
        eventRunMinutes: minutesBetween(
          event.eventStartedAt,
          event.eventEndedAt || stamp,
        ),
    ...
    finishBreakdown(eventId) {
      const event = Events.get(eventId);
      if (!event) return;
      if (!event.breakdownStartedAt)
        return alert("End the event first to start breakdown timing.");
      const stamp = nowIso();
      const breakdownEnd = event.breakdownEndedAt || stamp;
      const setupMinutes = minutesBetween(
        event.setupStartedAt,
        event.eventStartedAt,
      );
      const eventRunMinutes = minutesBetween(
        event.eventStartedAt,
        event.eventEndedAt,
      );
      const breakdownMinutes = minutesBetween(
        event.breakdownStartedAt,
        breakdownEnd,
      );
      Events.update(eventId, {
        ...
        totalOperationalMinutes:
          setupMinutes + eventRunMinutes + breakdownMinutes,
        lifecycleStage: "finished",
        status: "Awaiting Closeout",
      });
      this.exit(false);
      UI.renderEventDetail(eventId);
      if (typeof UI.openEventCloseout === "function")
        setTimeout(() => UI.openEventCloseout(eventId), 0);
    },
    ...
    selectEvent(eventId) {
      ...
      this.activeId = eventId;
      localStorage.setItem(ACTIVE_KEY, eventId);
      if (["setup", "event", "breakdown"].includes(event.lifecycleStage))
        this.enter(eventId);
    ...
    applyFocusedLayout() {
      ...
      if (topbarRight) {
        ...
        topbarRight.style.gridTemplateColumns =
    ...
    renderStageControls(event) {
      ...
      if (stage === "setup")
        return `<button type="button" onclick="LiveEvent.markEventStarted('${UI.esc(event.id)}')">Event Has Started</button>`;
      if (stage === "event")
        return `<button type="button" onclick="LiveEvent.endEvent('${UI.esc(event.id)}')">End Event / Start Breakdown</button>`;
      if (stage === "breakdown")
        return `<button type="button" onclick="LiveEvent.finishBreakdown('${UI.esc(event.id)}')">Finish Breakdown</button>`;
      return "";
    },

    renderTiming(event) {
      const setupLive = event.eventStartedAt
        ? event.setupMinutes
        : minutesBetween(event.setupStartedAt, nowIso());
      const eventLive = event.eventEndedAt
        ? event.eventRunMinutes
        : event.eventStartedAt
          ? minutesBetween(event.eventStartedAt, nowIso())
          : 0;
      const breakdownLive = event.breakdownEndedAt
        ? event.breakdownMinutes
        : event.breakdownStartedAt
          ? minutesBetween(event.breakdownStartedAt, nowIso())
          : 0;
      const total =
        Number(setupLive || 0) +
        Number(eventLive || 0) +
        Number(breakdownLive || 0);
      return `
                <div class="card">
  ...
  UI.renderLayout = function (...args) {
    ...
    if (
      topbarRight &&
      !document.getElementById("topSalesButton") &&
      typeof SalesUI !== "undefined"
    ) {
    ...
    if (
      ...
      ["setup", "event", "breakdown"].includes(
        LiveEvent.activeEvent.lifecycleStage,
      )
    )
  ...
  UI.renderEventDetail = function (id) {
    ...
    if (card) {
      ...
      controls.innerHTML =
        event.setupStartedAt && !["finished"].includes(event.lifecycleStage)
