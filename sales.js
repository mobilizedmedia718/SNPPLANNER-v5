...
const SalesUI = {
    ...
    async renderPatronPicker() {
        ...
        if (!eventId) {
            workspace.innerHTML = `
                ...
                <div class="card">
                    <h3>Choose Event</h3>
                    <p>Sales starts with the event so SNP Planner can show only the patrons who are actually there.</p>
                    ${events.length ? events.map(e => `
                        <button type="button" style="margin:6px;" onclick="SalesUI.chooseEvent('${UI.esc(e.id)}')">
                            ${UI.esc(e.name || "Untitled Event")}${e.date ? ` — ${UI.esc(e.date)}` : ""}
                        </button>
                    `).join("") : `<p>No active events found.</p>`}
                </div>
            `;
            return;
        }
            ...
            <div class="card">
                ...
                <div id="salesPatronButtons">
                    ${patrons.length ? patrons.map(c => `
                        <button type="button" style="display:block;width:100%;margin:8px 0;padding:14px;text-align:left;" onclick="SalesUI.selectPatron('${UI.esc(c.id)}')">
                            <strong>${UI.esc(CRM.fullName(c) || c.email || "Patron")}</strong>${c.email ? `<br><small>${UI.esc(c.email)}</small>` : ""}
                        </button>
                    `).join("") : `<p>No checked-in patrons yet. Scan a ticket or refresh Eventbrite check-ins.</p>`}
                    <button type="button" style="display:block;width:100%;margin:8px 0;padding:14px;text-align:left;" onclick="SalesUI.selectPatron('')">
                        <strong>Walk-in / No Patron</strong>
                    </button>
                </div>
            </div>
        `;
    ...
    async syncPatrons() {
        ...
        try {
            await Eventbrite.syncCheckedIn(eventId);
            await this.renderPatronPicker();
        } catch (error) {
            alert(error?.message || "Unable to sync Eventbrite check-ins.");
        } finally {
            this.syncingPatrons = false;
        }
    },

    selectPatron(customerId) {
    ...
    subtractItem(id) {
        const current = Number(this.cart[id] || 0);
        this.cart[id] = Math.max(0, current - 1);
        this.renderMenu();
    },
    ...
    renderMenu() {
        ...
        workspace.innerHTML = `
            <h2>Event Sales Menu</h2>
            <div class="card">
                <p><strong>Event:</strong> ${UI.esc(event?.name || "General")}</p>
                <p><strong>Patron:</strong> ${UI.esc(customer ? (CRM.fullName(customer) || customer.email || "Patron") : "Walk-in")}</p>
                <button type="button" onclick="SalesUI.renderPatronPicker()">Change Patron</button>
            </div>

            <div class="card">
                <h3>Menu</h3>
                ${items.length ? `<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px;">${items.map(item => `
                    <button type="button" onclick="SalesUI.addItem('${UI.esc(item.id)}')" style="padding:16px;text-align:left;min-height:100px;">
                        <strong>${UI.esc(item.name || "Item")}</strong><br>
                        <span>${Utils.money(item.sellPrice)}</span><br>
                        <small>Stock: ${Number(item.quantity || 0)}</small>
                    </button>
                `).join("")}</div>` : `<p>No Event Sales inventory is available. In Inventory, set an item's Category to <strong>Event Sales</strong> and give it a Sell Price.</p>`}
            </div>

            <div class="card">
                <h3>Current Ticket</h3>
                ${this.selectedItems().length ? this.selectedItems().map(item => `
                    <div style="display:flex;justify-content:space-between;gap:10px;align-items:center;padding:8px 0;border-bottom:1px solid #eee;">
                        <span>${UI.esc(item.name)}</span>
                        <span>
                            <button type="button" onclick="SalesUI.subtractItem('${UI.esc(item.inventoryId)}')">−</button>
                            <strong style="padding:0 10px;">${item.quantity}</strong>
                            <button type="button" onclick="SalesUI.addItem('${UI.esc(item.inventoryId)}')">+</button>
                        </span>
                        <strong>${Utils.money(item.price * item.quantity)}</strong>
                    </div>
                `).join("") : `<p>No items added yet.</p>`}
                <br>
                <p style="font-size:1.3em;"><strong>Total: ${Utils.money(this.total())}</strong></p>
                <button type="button" onclick="SalesUI.checkout()" ${this.selectedItems().length ? "" : "disabled"}>Charge with Stripe</button>
                <button type="button" onclick="SalesUI.clearCart()">Clear Ticket</button>
            </div>
        `;
    },
    ...
    async checkout() {
        ...
        try {
            await SNPStripePayments.startCheckout(items, {
                eventId,
