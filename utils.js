const Utils = {
    ...
    money(value) {
        ...
        const currency = typeof Settings !== "undefined" && Settings.data && Settings.data.currency ? Settings.data.currency : "USD";
        try {
    ...
    searchAll(term) {
        ...
        if (!query) return { customers: [], events: [], vendors: [], venues: [], inventory: [], assets: [] };
        const matches = (...values) => values.flat().filter(value => value !== undefined && value !== null).join(" ").toLowerCase().includes(query);
        return {
            customers: CRM.all().filter(c => matches(c.firstName, c.lastName, c.email, c.phone, c.tags || [])),
            events: Events.all().filter(e => matches(e.name, e.status, e.theme, e.instructor, e.notes)),
            vendors: Vendors.all().filter(v => matches(v.name, v.category, v.contact, v.email, v.phone)),
            venues: Venues.all().filter(v => matches(v.name, v.address, v.city, v.state, v.zip)),
            inventory: Inventory.all().filter(i => matches(i.name, i.category, i.sku, i.storageLocation)),
            assets: Assets.all().filter(a => matches(a.name, a.category, a.serialNumber, a.location, a.assignedTo))
        };
    },
        ...
        reader.onload = async event => {
            try {
                ...
                entries.forEach(([key, value, valid]) => {
                    ...
                    if (window.SNPDatabase?.user?.()) cloudWrites.push(SNPDatabase.saveStorage(key, value));
                });
