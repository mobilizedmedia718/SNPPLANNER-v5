(function () {
  const DEFAULT_EVENT_URL =
    "https://www.eventbrite.com/e/paint-the-town-a-sip-and-paint-experience-tickets-1995025559152?aff=oddtdtcreator";
  ...
  const META_CONNECTION_DEFAULTS = {
    ...
    connectorUrl:
      "https://mmstqostdqouxaiyrxtv.supabase.co/functions/v1/meta-promo-connector",
    ...
    lastStatusDetail:
      "Create the Facebook Page, connect Instagram, then test a secure connector when it is ready.",
    lastTestAt: "",
  };
  ...
  const DEFAULT_STATE = {
    ...
    dailyPostGoal: 3,
    targetAudience:
      "Oakland and Bay Area adults, creatives, couples, friends, date-night guests, and first-time painters",
    offer: "Tickets start at $40. Limited seats available.",
    eventUrl: DEFAULT_EVENT_URL,
    approvalRequired: true,
    hashtags:
      "#PaintTheTown #OaklandEvents #SipAndPaint #OaklandNightlife #BayAreaEvents #OaklandArt #DateNightOakland #ThingsToDoInOakland #EastOakland #BlackOwnedEvents #CreativeNightOut #PaintAndSip",
  ...
  const PromoAgent = {
    ...
    load() {
      const saved =
        typeof Utils !== "undefined" ? Utils.load(STORAGE_KEY, {}) : {};
      this.state = {
        ...DEFAULT_STATE,
        ...(saved || {}),
        creative: {
          ...CREATIVE_DEFAULTS,
          ...(saved?.creative || {}),
          referenceFiles: Array.isArray(saved?.creative?.referenceFiles)
            ? saved.creative.referenceFiles
            : [],
        },
        metaConnection: {
        ...
        mediaLibrary: Array.isArray(saved?.mediaLibrary)
          ? saved.mediaLibrary
          : [],
    ...
    esc(value) {
      if (typeof UI !== "undefined" && typeof UI.esc === "function")
        return UI.esc(value);
      return String(value ?? "")
        .replace(/&/g, "&amp;")
        .replace(/</g, "&lt;")
    ...
    events() {
      return typeof Events !== "undefined" && typeof Events.all === "function"
        ? Events.all()
        : [];
    },
    ...
    updateMetaConnection(field, value) {
      if (!this.state.metaConnection)
        this.state.metaConnection = { ...META_CONNECTION_DEFAULTS };
      if (!(field in META_CONNECTION_DEFAULTS)) return;
      if (
        [
          "instagramBusinessReady",
          "facebookPageReady",
          "businessSuiteReady",
          "twoFactorReady",
          "allowConnectorPublishing",
        ].includes(field)
      ) {
    ...
    async testMetaConnector() {
      ...
      if (!url) {
        alert(
          "Add the secure connector URL first. This should be a Supabase Edge Function or Cloudflare Worker, not an Instagram password.",
        );
        return;
      }
      ...
      try {
        ...
        const message =
          ...
          data.account ||
          (response.ok
            ? "Connector answered successfully."
            : "Connector rejected the test.");
    ...
    async sendToMetaConnector(id) {
      ...
      if (!this.canSendToConnector(item)) {
        alert(
          "This item is not an Instagram/Facebook post. Copy it or use the matching platform manually.",
        );
        return;
      }
      if (!item.previewedAt) {
        this.openPreview(id);
        alert(
          "Review the visual preview first. After it looks right, approve it before sending to the connector.",
        );
        return;
      }
      if (item.status !== "Approved" && item.status !== "Sent to Connector") {
        alert(
          "Approve this post first. The agent will not send anything blind.",
        );
        return;
      }
      ...
      if (!this.connectorReady()) {
        alert(
          "The Instagram/Facebook connector is not ready yet. Complete the checklist and run a successful connector test first.",
        );
        return;
      }
      if (!connection.allowConnectorPublishing) {
        alert(
          "Turn on 'Allow connector to publish approved posts' before sending live content.",
        );
        return;
      }
    ...
    storageHeaders(contentType) {
      ...
      if (
        !token ||
        typeof SNPDatabase === "undefined" ||
        typeof SNPDatabase.headers !== "function"
      )
        return null;
    ...
    mediaLibrary() {
      return Array.isArray(this.state.mediaLibrary)
        ? this.state.mediaLibrary
        : [];
    },
    ...
    mediaForQueue(item) {
      ...
      return (
        this.mediaLibrary().find(
          (media) => media.publicUrl === item.publicMediaUrl,
        ) || null
      );
    },
    ...
    async uploadPromoMedia(input) {
      ...
      for (const file of files.slice(0, 10)) {
        if (!PROMO_MEDIA_TYPES.includes(file.type)) {
          alert(
            `${file.name} is not an allowed media type. Use JPG, PNG, WebP, GIF, MP4, MOV, or WebM.`,
          );
          continue;
        }
        ...
        const response = await fetch(
          `${this.supabaseUrl()}/storage/v1/object/${PROMO_MEDIA_BUCKET}/${this.encodeStoragePath(path)}`,
          {
            method: "POST",
            headers,
            body: file,
        ...
        if (!response.ok) {
          ...
          alert(
            `Upload failed for ${file.name}: ${detail || response.statusText}`,
          );
          continue;
        }
      ...
      if (uploaded.length) {
        this.state.mediaLibrary = [...uploaded, ...this.mediaLibrary()].slice(
          0,
          80,
        );
        this.state.log = [
          {
            ...
            message: `Uploaded ${uploaded.length} promo media file(s) to Supabase Storage`,
          },
          ...this.state.log,
        ].slice(0, 25);
        this.save();
    ...
    removeMediaFromLibrary(id) {
      const media = this.mediaById(id);
      if (!media) return;
      const attached = this.state.queue.some(
        (item) => item.publicMediaUrl === media.publicUrl,
      );
      if (
        attached &&
        !confirm(
          "This media is attached to a queued post. Remove it from the media library anyway? The public URL will stay attached to the post.",
        )
      )
        return;
      this.state.mediaLibrary = this.mediaLibrary().filter(
        (row) => row.id !== id,
      );
      this.save();
      this.render();
    ...
    async handleReferenceUpload(input) {
      ...
      if (!files.length) return;
      if (!this.state.creative) this.state.creative = { ...CREATIVE_DEFAULTS };
      const current = Array.isArray(this.state.creative.referenceFiles)
        ? this.state.creative.referenceFiles
        : [];
      const accepted = [];
      ...
      for (const file of files.slice(0, 5)) {
        if (file.size > maxBytes) {
          alert(
            `${file.name} is too large for planner storage. Use files under about 1.5 MB for now.`,
          );
          continue;
        }
      ...
      this.state.creative.referenceFiles = [...accepted, ...current].slice(
        0,
        8,
      );
    ...
    removeReference(id) {
      ...
      if (this.state.creative.primaryReferenceId === id) {
        this.state.creative.primaryReferenceId =
          this.state.creative.referenceFiles[0]?.id || "";
      }
      this.save();
      this.render();
    },
      ...
      if (Array.isArray(event.ticketTypes)) {
        count = Math.max(
          ...
          event.ticketTypes.reduce(
            (sum, item) => sum + Number(item.sold || item.quantitySold || 0),
            0,
          ),
        );
      }
      if (
        typeof Eventbrite !== "undefined" &&
        typeof Eventbrite.link === "function"
      ) {
        const link = Eventbrite.link(event.id);
        count = Math.max(count, Number(link.manualTicketsSold || 0));
        if (Array.isArray(link.attendees))
          count = Math.max(count, link.attendees.length);
      }
      return count;
    },
    ...
    strategyFor(phase) {
      const strategies = {
        Awareness:
          "Show the vibe, the venue, the painting, and the easy beginner-friendly experience. Push shareable content and local discovery.",
        Conversion:
        ...
        "Sellout push":
          "Stop broad discounting. Push waitlist, next event interest, and brand-building content.",
    ...
    eventFacts(event) {
      const venue =
        typeof Venues !== "undefined" && event?.venueId
      ...
      const location = venue
        ...
        : "Oakland";
      const eventName =
        event?.name || "Paint The Town - A Sip & Paint Experience";
      const date = this.formatDate(event?.date);
      const time =
        [event?.time, event?.endTime]
          .filter(Boolean)
          .map((value) => Utils.formatTime(value))
          .join(" – ") || "4:00 PM – 7:00 PM";
    ...
    primaryReference() {
      ...
      return (
        files.find(
          (file) => file.id === this.state.creative?.primaryReferenceId,
        ) ||
        files.find((file) => String(file.type || "").startsWith("image/")) ||
        files[0] ||
        null
      );
    },
    ...
    creativeSnapshot() {
      ...
      return {
        ...
        referenceNames: (creative.referenceFiles || []).map(
          (file) => file.name,
        ),
      };
    },
    ...
    productionNotes() {
      ...
      if (snap.primaryReferenceName)
        lines.push(`Primary reference: ${snap.primaryReferenceName}`);
      if (snap.visualDirection)
        lines.push(`User visual direction: ${snap.visualDirection}`);
      if (snap.musicMode === "Agent decides") {
        lines.push(
          `Music: Agent chooses a track that matches ${snap.musicStyle || "the event vibe"}.`,
        );
      ...
      if (snap.extraInstructions)
        lines.push(`Extra instructions: ${snap.extraInstructions}`);
      return lines.join("\n");
    },
    ...
    buildPlan(event) {
      ...
      const dailyTarget =
        gap > 0 ? Math.ceil(gap / Math.max(daysLeft || 1, 1)) : 0;
      ...
      const audience =
        this.state.targetAudience || DEFAULT_STATE.targetAudience;
      const localAngle = facts.location.toLowerCase().includes("oakland")
        ? "Oakland"
        : "local";
        ...
        content: [
          {
            ...
            title: "Main caption",
            copy: this.postBrief(caption),
          },
          {
            channel: "Instagram Story",
            title: "Story sequence",
            copy: this.postBrief(story),
          },
          {
            channel: "Facebook Groups",
            title: "Local group post",
            copy: this.postBrief(facebook),
          },
          { channel: "DM Outreach", title: "Personal invite script", copy: dm },
          {
            channel: "Eventbrite",
            title: "Event update",
            copy: this.postBrief(eventbriteUpdate),
          },
          {
            channel: "Eventbrite Ads",
            title: "Ad draft",
            copy: this.postBrief(adCopy),
          },
        ],
    ...
    tasksFor(phase, dailyTarget, adBudget) {
      ...
      if (phase === "Urgency" || phase === "Last call") {
        ...
        base.push(
          "Message every warm lead who reacted, liked, or asked questions.",
        );
      }
      ...
      this.state.log = [
        {
          id: Utils.id(),
          at: now,
          message: `Ran ${plan.phase} plan for ${plan.eventName}`,
        },
        ...this.state.log,
      ].slice(0, 25);
      this.save();
      ...
      this.state.log = [
        {
          id: Utils.id(),
          at: new Date().toISOString(),
          message: `Added ${plan.tasks.length} promo tasks to Calendar`,
        },
        ...this.state.log,
      ].slice(0, 25);
      this.save();
    ...
    clearGeneratedAds() {
      const hasGeneratedContent =
        this.state.queue.length || this.state.lastPlan || this.state.lastRun;
      ...
      const approved = confirm(
        "Clear all generated ads and the last promo plan? Your uploaded media, connector settings, and event settings will stay saved.",
      );
      if (!approved) return;
      ...
      this.state.log = [
        {
          id: Utils.id(),
          at: now,
          message: "Cleared generated promo approval queue to start fresh",
        },
        ...this.state.log,
      ].slice(0, 25);
      this.save();
      ...
      return `
                <div class="promo-agent-social-preview">
                    ...
                    <div>
                        ...
                        ${item.previewedAt ? `<p class="promo-agent-preview-note">Last preview opened: ${this.esc(Utils.formatDateTime(item.previewedAt))}</p>` : ""}
                    </div>
                </div>
            `;
    },
...
${references}
    ...
    referencePreviewHtml() {
      const files = this.state.creative?.referenceFiles || [];
      if (!files.length) return "";
      return `<h2>Reference Files</h2><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:10px;">${files
        .map((file) => {
    ...
    openPreview(id) {
      ...
      this.render();
      if (!opened)
        prompt("Popup was blocked. Copy and open this preview link:", url);
    },
    ...
    approveQueue(id) {
      ...
      if (!item.previewedAt) {
        this.openPreview(id);
        alert(
          "Review the visual preview first. After it looks right, click Approve again.",
        );
        return;
      }
      this.updateQueue(id, {
    ...
    renderReferenceFiles() {
      const files = this.state.creative?.referenceFiles || [];
      if (!files.length)
        return `<p class="promo-agent-muted">No reference files uploaded yet.</p>`;
      return `
    ...
    renderMetaConnectionControls() {
      ...
      const tested = connection.lastTestAt
        ? Utils.formatDateTime(connection.lastTestAt)
        : "Not tested yet";
      return `
                <div class="card">
                    <div class="promo-agent-item-head">
